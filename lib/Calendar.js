...
var Calendar = (function (_Component) {
  ...
  _createClass(Calendar, [{
    ...
    key: 'isDayPassive',
    value: function isDayPassive(monthStart, monthEnd) {
      var _this = this;

      return function (day) {
        var _props4 = _this.props;
        var disableDay = _props4.disableDay;
        ...
        minDate = minDate && (0, _utilsParseInputJs2['default'])(minDate, format) || monthStart;
        maxDate = maxDate && (0, _utilsParseInputJs2['default'])(maxDate, format) || monthEnd;
    ...
    value: function renderDays() {
      ...
      var startOfMonth = shownDate.clone().startOf('month');
      ...
      var diff = Math.abs(firstDayOfWeek - (startOfMonth.isoWeekday() + 7)) % 7;
      var startDate = startOfMonth.clone().add(-diff, 'd');
      var isDayPassive = this.isDayPassive(startOfMonth, shownDate.clone().endOf('month'));

      for (var i = 0; i < 43; i++) {
        var dayMoment = startDate.clone().add(i, 'd');
        days.push({ dayMoment: dayMoment, isPassive: isDayPassive(dayMoment) });
      }
...
Calendar.propTypes = {
  ...
  disableDay: _react.PropTypes.func,
