...
var Calendar = (function (_Component) {
  ...
  _createClass(Calendar, [{
    key: 'getDayText',
    value: function getDayText(dayMoment) {
      var len = this.dayText.length;
      for (var i = 0; i < len; i++) {
        var _m = (0, _moment2['default'])(this.dayText[i].ts, 'x');
    ...
    value: function renderDays(classes) {
      ...
      for (var i = diff - 1; i >= 0; i--) {
        ...
        var _dayText = this.showText && this.getDayText(dayMoment) || null;
        days.push({ dayMoment: dayMoment, dayText: _dayText, isPassive: true });
      }

      // Current month's days
      for (var i = 1; i <= dayCount; i++) {
        var dayMoment = shownDate.clone().date(i);
        var _dayText2 = this.showText && this.getDayText(dayMoment) || null;
        ...
        if (disableDaysBeforeToday && Number(dayMoment.diff(_today, "days")) <= -1) {
          days.push({ dayMoment: dayMoment, dayText: _dayText2, isPassive: true });
        } else {
          days.push({ dayMoment: dayMoment, dayText: _dayText2 });
        }
      }
      ...
      for (var i = 1; i <= remainingCells; i++) {
        ...
        var _dayText3 = this.showText && this.getDayText(dayMoment) || null;
        days.push({ dayMoment: dayMoment, dayText: _dayText3, isPassive: true });
      }
      ...
      return days.map(function (data, index) {
        ...
        return _react2['default'].createElement(_DayCellJs2['default'], _extends({
          ...
          showText: _this.showText ? true : false,
          theme: styles,
...
Calendar.propTypes = {
  ...
  dayText: _react.PropTypes.array
};
