...
var Calendar = (function (_Component) {
  ...
  _createClass(Calendar, [{
    ...
    value: function renderMonthAndYear(classes) {
      ...
      var month = _moment2['default'].localeData(locale).months()[shownDate.month()];
    ...
    value: function renderWeekdays(classes) {
      ...
      var weekdays = [].concat(_toConsumableArray(localeData.weekdaysMin()));
      var firstDays = weekdays.splice(0, firstDayOfWeek);
    ...
    value: function renderDays(classes) {
      ...
      var firstDayOfWeek = _moment2['default'].localeData(locale).firstDayOfWeek();
      var shownDate = this.getShownDate();
      var date = this.state.date;
      ...
      for (var _i = diff - 1; _i >= 0; _i--) {
        var dayMoment = lastMonth.clone().date(lastMonthDayCount - _i);
        days.push({ dayMoment: dayMoment, isPassive: true });
      }

      // Current month's days
      for (var _i2 = 1; _i2 <= dayCount; _i2++) {
        var dayMoment = shownDate.clone().date(_i2);
      ...
      for (var _i3 = 1; _i3 <= remainingCells; _i3++) {
        var dayMoment = nextMonth.clone().date(_i3);
        days.push({ dayMoment: dayMoment, isPassive: true });
      }
